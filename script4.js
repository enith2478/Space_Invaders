...
class Enemy{
    ...
    if (this.lives < 1){
        ...
        if (this.frameX > this.maxFrame){
            ...
            if (!this.gameOver)this.game.score += this.maxLives;
    }
}
...
class Boss {
    ...
    update(){
        ...
        // collision deetection boss projectiles
        this.game.projectilesPool.forEach(projectile => {
...
class Wave {
    ...
    render(context){
        ...
        if (this.enemies.lenght <= 0) this.markedForDeletion = true;
    }
    create(){
...
class Game{
    constructor(canvas){
        ...
        window.addEventListener('keydown', e => {
            ...
            if (e.key === 'r' || e.key  === 'Enter' && this.gameOver) this.restart();
        });
    ...
    //create projectiles objects pool
    createProjectiles(){
    ...
    drawStatusText(context){
       ...
       context.shadowColor = 'black';
        ...
        if (this.gameOver){
            ...
            context.fillText('press R or Enter to restart!', this.width * 0.5, this.height * 0.5 + 30);
        }
        context.restore();
    }
    newWave(){
